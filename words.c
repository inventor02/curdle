...
/**
 * Calculates and returns today's word.
 *
 * @return today's word
 */
char *get_today_word() {
...
/**
 * Checks whether a word is a valid guess or not.
 *
 * @param word the word to check against the possible guesses list
 * @return whether the word is a valid guess or not
 */
bool is_valid_guess(char *word) {
  bool found = false;
  for (uint32_t i; !found && i < CURDLE_GUESS_LIST_LENGTH; i++) {
    if (possible_guess_words[i] == word) {
      found = true;
    }
  }
